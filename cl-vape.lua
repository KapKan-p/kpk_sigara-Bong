	...
	if DoesEntityExist(ped) and not IsEntityDead(ped) then
		if IsPedOnFoot(ped) then
			if IsPlayerAbleToVape == false then
				...
				ShowNotification("Elektronik sigaranı kullanmaya başladın.")
				Wait(Config.HelpTextLength)
				TriggerEvent("Vape:HelpFadeOut", 0)
			else
				ShowNotification("Elektronik sigarayı zaten tutuyorsun.")
			end
		else
			ShowNotification("Araç içinde bunu yapamazsın")
		end
	else
		ShowNotification("Yaralıysan bunu yapamazsın.")
	end
end)
...
AddEventHandler("Vape:StopVaping", function(source)
	if IsPlayerAbleToVape == true then
		...
		ShowNotification("Elektronik sigarayı bıraktın.")
	else
		ShowNotification("Elektronik sigara tutmuyorsun.")
	end
end)
AddEventHandler("Vape:Drag", function()
	if IsPlayerAbleToVape then
		...
		ShowNotification("Bunu yapmak için elektronik sigarayı tutman lazım.")
	end
end)
...
AddEventHandler("Bong:Drag", function()
	if IsPlayerAbleToBong then
		...
		ShowNotification("Bunu yapmak için Bong tutman lazım.")
	end
end)
...
AddEventHandler("Bong:StopVaping", function(source)
	if IsPlayerAbleToBong == true then
		...
		ShowNotification("Bongu bıraktın.")
	else
		ShowNotification("Bong tutmuyorsun.")
	end
end)
	...
	if DoesEntityExist(ped) and not IsEntityDead(ped) then
		if IsPedOnFoot(ped) then
			if IsPlayerAbleToBong == false then
				PlayerIsAbleToBong()
				TriggerEvent("Vape:HelpFadeIn", 0)
				ShowNotification("Bong kullanmaya başladın.")
				Wait(Config.HelpTextLength)
				TriggerEvent("Vape:HelpFadeOut", 0)
			else
				ShowNotification("Bongu zaten tutuyorsun.")
			end
		else
			ShowNotification("Araç içinde bunu yapamazsın")
		end
...
AddEventHandler("Sigara:StopVaping", function(source)
	if IsPlayerAbleToSigara == true then
		...
		ShowNotification("Sigarayı bıraktın.")
	else
		ShowNotification("Sigara tutmuyorsun.")
	end
end)
	...
	if DoesEntityExist(ped) and not IsEntityDead(ped) then
		if IsPedOnFoot(ped) then
			if IsPlayerAbleToSigara == false then
				...
				ShowNotification("Sigarayı zaten tutuyorsun.")
			end
		else
			ShowNotification("Araç içinde bunu yapamazsın")
		end
	...
	while true do
		if IsPlayerAbleToVape then
			if DisplayText then
				local ped = GetPlayerPed(-1)
				local pedPos = GetEntityCoords(ped)
				DrawText3d(pedPos.x, pedPos.y, pedPos.z+1.20, " ~b~\"E\"~c~ tuşuna basılı tutarak içine çek.")
				DrawText3d(pedPos.x, pedPos.y, pedPos.z+1.08, " ~b~\"G\"~c~ tuşuna basılı tutarak içmeyi bırak.")
			end
		end
	  Wait(1)
	end
...
function PlayerIsAbleToSigara()
	...
	local x,y,z = table.unpack(GetEntityCoords(ped))
	local sigara = CreateObject(GetHashKey(prop_name), x, y, z + 0.2, true, true, true)
...
function ShowNotification( text )
	exports['mythic_notify']:DoHudText('inform', text, 8000)
